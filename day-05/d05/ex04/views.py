...
def create_table(request):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("CREATE TABLE ex04_movies ( \
...
def populate(request):
    ...
    for i in data:
        cmd = f"INSERT INTO ex04_movies (title, episode_nb, director, producer, release_date) VALUES (\
...
def display(request):
    ...
    cursor.close()
    conn.close()
    return render(request, 'ex04/display_movies.html', {'data': data})
...
def remove_page(request):
    ...
    cursor.close()
    conn.close()
    return render(request, 'ex04/delete_movies.html', {'data': data})
...
def remove_movie(request):
    ...
    try:
        conn = psycopg2.connect('dbname=djangotraining user=djangouser password=secret')
        cursor = conn.cursor(cursor_factory = psycopg2.extras.RealDictCursor)
        cursor.execute(f"DELETE from ex04_movies WHERE episode_nb = {request.POST['movies']};")
    except Exception as err:
        return HttpResponse("No data available")
