...
def display(request):
    ...
    return render(request, 'ex07/display_movies.html', {'data': data})

def update_page(request, data):
    return render(request, 'ex07/update_movies.html', {'data': data})
...
def update(request):
    ...
    if request.method != 'POST':
        return update_page(request, data)

    update_str = Movies.objects.get(episode_nb=request.POST['movies'])
    update_str.opening_crawl = request.POST['text']
    update_str.save()
